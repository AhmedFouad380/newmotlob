...
class BlockReasonController extends Controller
{
    ...
    {
        $data = BlockReason::orderBy('id', 'desc');
    ...
    public function update(Request $request){
        ...
        $data =  BlockReason::find($request->id);
        $data->reason=$request->reason;
        $data->is_active=$request->is_active;
    ...
    {
        $employee = BlockReason::findOrFail($id);
        return view('admin.BlockReason.edit', compact('employee'));
    }

    public function destroy(Request $request)
    {
        try {
            BlockReason::whereIn('id', $request->id)->delete();
        } catch (\Exception $e) {
